...
document.getElementById('submit-btn').addEventListener('click',()=>{
  const name=document.getElementById('name').value,email=document.getElementById('email').value,city=document.getElementById('city').value;
  ...
  if(!name||!email||!city||!bio||!specialties.length)return showToast('Please fill required fields','error');
  const guide=addGuide({name,email,city,bio,price,languages,specialties});
